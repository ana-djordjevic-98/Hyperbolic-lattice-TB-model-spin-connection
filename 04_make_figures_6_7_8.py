...
def fig6(data):
    ...
    # global max curve height is 0.893 (verified separately): extend
    # ylim to open a genuinely empty band at the top (0.90-1.15) for the
    # inset, rather than overlaying it on top of any data.
    ax.set_ylim(0, 1.15)
    ax.legend(fontsize=7, ncol=2, loc="upper right")

    # --- inset: rho_0(E) near E=0 for the largest system (N=571210,
    # enhanced R=48 statistics computed separately from the official
    # Table I checkpoint). Shown as a function of E itself (linear axis,
    # signed, not folded onto |E|), with rho_0 on a log axis. The fitted
    # power law rho_0(E)=C|E|^alpha, a single fit using both signs of E
    # together (doubling the statistics; also a left/right consistency
    # check), appears in this view as its natural symmetric "V": two
    # line segments of equal and opposite slope meeting at E=0, since
    # log(rho_0) = alpha*log|E| + log C is linear in log|E|, not in E
    # itself. Window: the full symmetric range -0.01<E<0.01, down to the
    # KPM resolution floor. ---
    sym_path = os.path.join(ROOT, "results", "scaling_analysis",
                              "dos_H0_N571210_symmetric.npz")
    if os.path.exists(sym_path):
        ds = np.load(sym_path)
        Es, rs, ss = ds["E"], ds["rho0"], ds["rho0_sem"]
        alpha, alpha_sem, intercept = (float(ds["alpha"]), float(ds["alpha_sem"]),
                                         float(ds["intercept"]))
        floor = 0.0007  # ~ a_scale/M for N=571210
        m = np.abs(Es) > floor

        axins = ax.inset_axes([0.12, 0.79, 0.40, 0.19])
        axins.set_facecolor("white")
        axins.set_yscale("log")
        axins.plot(Es[m], rs[m], "-", color="black", lw=1.1, zorder=3)
        axins.fill_between(Es[m], np.clip(rs[m] - ss[m], 1e-12, None),
                             rs[m] + ss[m], color="gray", alpha=0.3, zorder=2)
        Efit_pos = np.linspace(floor, 0.01, 20)
        Efit_neg = -Efit_pos[::-1]
        axins.plot(Efit_pos, np.exp(intercept) * Efit_pos**alpha, "--",
                    color="tab:red", lw=1.4, zorder=4,
                    label=rf"$\alpha={alpha:.2f}\pm{alpha_sem:.2f}$")
        axins.plot(Efit_neg, np.exp(intercept) * np.abs(Efit_neg)**alpha, "--",
                    color="tab:red", lw=1.4, zorder=4)
        axins.set_xlim(-0.01, 0.01)
        axins.set_xticks([-0.01, 0, 0.01])
        axins.tick_params(labelsize=6)
        axins.set_ylabel(r"$\rho_0$", fontsize=7.5, labelpad=1)
        axins.set_xlabel(r"$E$", fontsize=7.5, labelpad=1)
        axins.legend(fontsize=5.3, frameon=False, loc="lower right",
